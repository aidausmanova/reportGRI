...
def get_gri_standards(path):
    ...
    gri_list = []
    for gri in gri_data:
        gri_standards_corpus[gri["idx"]] = (
            f"{gri['title']} ({', '.join(gri['topics'])}). {gri['description']}"
        )
...
def get_gri_disclosures(path):
    ...
    for gri in gri_data:
        ...
        for disclosure in disclosures:
            gri_disclosures_corpus[disclosure["idx"]] = (
                f"{disclosure['title']} ({disclosure['description']})"
            )
            gri_disclosures_list.append(
                f"{disclosure['idx']}: {disclosure['description']}"
            )
...
def get_section_passages(corpus):
    ...
    section_corpus = {cur_section: f"{corpus[0]['title']}\n{corpus[0]['text']}"}
    section_data.append(
        {
            "section_idx": cur_section,
            "title": cur_title,
            "text": corpus[0]["text"],
            "gri": [],
        }
...
def process_llm_response(report_name):
    ...
    for gri_standard in tqdm(gri_data):
        ...
        for gri_disclosure in gri_standard["disclosures"]:
            gri_disclosure_titles[gri_disclosure["idx"]] = (
                gri_disclosure["code"],
                gri_disclosure["title"],
            )

    completeness_pattern = re.compile(
        r"""(?i)completeness[\\]?[\s"']*[:=]?[\s"']*(?P<completeness>\d+)""", re.VERBOSE
    )
    materiality_pattern = re.compile(
        r"""(?i)materiality[\\]?[\s"']*[:=]?[\s"']*(?P<materiality>\d+)""", re.VERBOSE
    )
    ...
    for row in data:
        ...
        for section_id in row["section_ids"]:
            ...
            paragraphs.append(
                {"section_idx": section_id, "title": section[0], "text": section[1]}
            )

        comment += (
            output_text.replace("comment", "")
            .replace("Comment", "")
            .replace('"', "")
            .replace(":", "")
            .replace(",\n", "")
