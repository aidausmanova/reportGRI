...
def process_openai(
    section_corpus, gri_standards, gri_disclosures, model, report_name, is_few_shot
):
    ...
    for paragraph in tqdm(section_corpus):
        standard_messages = message_format(
            gri_standard_prompt,
            is_few_shot,
        ...
        with open(
            f"data/gri_matches/{model}/{report_name}/gri_standards.json", "a"
        ) as f:
            f.write(json.dumps(standard_output_entry))
            f.write("\n")

        for gri_standard in gri_matchings:
            disclosure_messages = message_format(
                gri_disclosure_prompt.format(""),
                is_few_shot,
                {
            ...
            with open(f"output/{report_name}/gri_discosures.json", "a") as f:
                ...
                f.write("\n")
    print(
        f"[INFO] GRI matches are saved at data/gri_matches/{model}/gri_completeness_{model}.json"
    )
...
def process_causal_lm(section_corpus, gri_docs, model, tokenizer, report_name):
    ...
    for entry in tqdm(section_corpus):
        messages = [
            {"role": "system", "content": prompt_instructions + gri_instructions},
...
def evaluate_disclosure_coverage(
    report_name,
    retrieval_results,
    gri_disclosure_corpus,
