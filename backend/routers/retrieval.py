...
def run_retrieval(report_name):
    ...
    corpus_path = f"data/reports/{report_name}/{report_name}_corpus.json"
    with open(corpus_path, "r") as f:
        section_corpus = json.load(f)
    section_index_corpus = {}
    for passage in section_corpus:
        section_index_corpus[passage['section_idx']] = f"{passage['title']}\n{passage['text']}"
    ...
    for gri_disclosure in top_similarity_df['gri_disclosure'].unique():
        section_ids = list(top_similarity_df[top_similarity_df.gri_disclosure == gri_disclosure]['section_idx'].tolist())
        scores = list(set(top_similarity_df[top_similarity_df.gri_disclosure == gri_disclosure]['score'].tolist()))
        data_dict.append({
    ...
    with open(f'data/reports/{report_name}/top_retrieved_paragraphs.json', 'w') as f:
        json.dump(data_dict, f)
    print(f"[INFO] Retrieval output saved to data/reports/{report_name}/top_retrieved_paragraphs.json")
